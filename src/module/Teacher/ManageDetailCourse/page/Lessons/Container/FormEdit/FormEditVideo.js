...
import { actEditVideo } from '../../../../reducers/action';
export default function FormEditVideo({ onClose, value }) {
    ...
    React.useEffect(() => {
        if (value) {
            ...
            setValueURL(value.url_video)
        }
    }, [value, form]);
    const onFinish = (values) => {
        console.log(values)
        dispatch(actEditVideo(value.id, detailLesson.id, values, () => {
            form.resetFields()
            onClose()
        }))
    }
